...
// === Associated signs → clickable toggle buttons under <legend> ===
(function(){
  function ready(fn){ if(document.readyState!=='loading') fn(); else document.addEventListener('DOMContentLoaded',fn); }

  ready(function(){
    // Find the legend that literally says "Associated signs"
    const legends = Array.from(document.querySelectorAll('legend'));
    const legend = legends.find(l => (l.textContent||'').trim().toLowerCase() === 'associated signs');
    if(!legend) return;

    // The section area (fieldset preferred)
    const section = legend.closest('fieldset') || legend.parentElement;

    // Create/ensure a row for our buttons just after the legend
    let row = section.querySelector('.sai-as-buttons');
    if(!row){
      row = document.createElement('div');
      row.className = 'signs-container sai-as-buttons';
      legend.insertAdjacentElement('afterend', row);
    }

    // Hidden input to serialize selection if forms/analyze need it
    let hidden = section.querySelector('#hidden-signs');
    if(!hidden){
      hidden = document.createElement('input');
      hidden.type = 'hidden';
      hidden.id   = 'hidden-signs';
      hidden.name = 'associated_signs';
      legend.insertAdjacentElement('afterend', hidden);
    }

    // Canonical labels (order)
    const LABELS = ['Ptosis','Anisocoria','Pain with EOM','Proptosis'];

    // Hide any plain-text duplicates in the section (keeps UI clean)
    LABELS.forEach(L=>{
      const rx = new RegExp('^\\s*'+L.replace(/[.*+?^${}()|[\\]\\\\]/g,'\\$&')+'\\s*$', 'i');
      Array.from(section.querySelectorAll('p,li,span,div')).forEach(el=>{
        if(rx.test((el.textContent||'').trim())) el.style.display='none';
    ...
    // Build buttons (idempotent)
    LABELS.forEach(label=>{
      if (row.querySelector('[data-sign="'+label+'"]')) return;
      const b = document.createElement('button');
      b.type = 'button';
      b.className = 'sign-btn';
      b.dataset.sign = label;
    ...
    function update(){
      const selected = Array.from(row.querySelectorAll('.sign-btn.active')).map(b=>b.dataset.sign);
      hidden.value = selected.join(',');
      window.__sai_selected_signs = selected;
      window.dispatchEvent(new CustomEvent('associated-signs:change',{detail:{selected}}));
      // If a top chip exists, update its text
      const chip = document.getElementById('assoc-chip');
      if(chip){
        chip.textContent = selected.length ? ('Associated signs: '+selected.length+' set') : 'Associated signs: none';
    ...
    // Ensure analyze() reads latest selection
    if(typeof window.analyze === 'function'){
      const orig = window.analyze;
      if(!orig.__saiWrappedForAssoc){
        const wrapped = function(){ update(); return orig.apply(this, arguments); };
        wrapped.__saiWrappedForAssoc = true;
...
// === Remove previous "Associated signs" list so only buttons show ===
(function(){
  function ready(fn){ if(document.readyState!=='loading') fn(); else document.addEventListener('DOMContentLoaded',fn); }
  ready(function(){
    const legends = Array.from(document.querySelectorAll('legend'));
    ...
    const section = legend.closest('fieldset,section,article,div') || legend.parentElement;
    if(!section) return;

    const LABELS = ['Ptosis','Anisocoria','Pain with EOM','Proptosis'];
    const LSET = new Set(LABELS.map(s=>s.toLowerCase()));

    // Helper: does element contain any of our labels (rough, but safe)
    const containsLabel = (el)=>{
      const t = (el.textContent||'').toLowerCase();
      for(const s of LSET){ if(t.includes(s)) return true; }
      return false;
    };

    // 2.1 Hide any UL/OL/div that looks like the old list (but keep our new .sai-as-buttons)
    Array.from(section.querySelectorAll('ul,ol,div')).forEach(el=>{
      if(el.classList.contains('sai-as-buttons')) return;        // keep new buttons
      if(el.id === 'hidden-signs') return;                       // keep hidden field
      if(containsLabel(el)){
        el.classList.add('sai-old-associated');
        el.setAttribute('aria-hidden','true');
      }
    });

    // 2.2 If there were old checkboxes with labels, disable + hide their wrappers
    Array.from(section.querySelectorAll('input[type="checkbox"]')).forEach(cb=>{
      let txt = '';
      const id = cb.id && cb.id.trim();
      if(id){
        const lab = section.querySelector(`label[for="${CSS.escape(id)}"]`);
      ...
      if(txt){
        ...
        for(const s of LSET){
          if(lower.includes(s)){
            cb.checked = false;
            cb.disabled = true;
            const wrap = cb.closest('li,div,label') || cb.parentElement;
            if(wrap){
              wrap.classList.add('sai-old-associated');
              wrap.setAttribute('aria-hidden','true');
            } else {
              cb.classList.add('sai-old-associated');
    ...
    // 2.3 If the legend has an immediately following descriptive list, hide it
    let sib = legend.nextElementSibling;
    if(sib && !sib.classList.contains('sai-as-buttons') && containsLabel(sib)){
      sib.classList.add('sai-old-associated');
...
// === Associated signs: remove legacy list to avoid duplicates ===
(function(){
  function ready(fn){ if(document.readyState!=='loading') fn(); else document.addEventListener('DOMContentLoaded',fn); }
  ready(function(){
    const LABELS = ['Ptosis','Anisocoria','Pain with EOM','Proptosis'];

    // Find the legend "Associated signs"
    const legend = Array.from(document.querySelectorAll('legend'))
      .find(l => (l.textContent||'').trim().toLowerCase() === 'associated signs');
    if(!legend) return;
    ...
    // Keep our new row
    const keepRow = section.querySelector('.sai-as-buttons');

    // Candidates that might be the old list
    const cands = Array.from(section.querySelectorAll('#associated-signs, #associated-signs-list, .associated-signs, .assoc-list, ul, ol, div, p'))
      .filter(el => el && el !== keepRow);

    // Helper: count label matches inside an element
    function labelCount(el){
      const txt = (el.textContent||'').toLowerCase();
      let n = 0;
      LABELS.forEach(L => { if(txt.includes(L.toLowerCase())) n++; });
      return n;
    }

    // Hide any element that contains 2+ labels (very likely the legacy bullet list),
    // but do NOT hide our .sai-as-buttons row.
    cands.forEach(el=>{
      if (keepRow && (keepRow.contains(el) || el.contains(keepRow))) return;
      if (labelCount(el) >= 2) {
